...
function normalizeExpireOptions(items) {
  ...
  fallbackSource.forEach((item, index) => {
    ...
    if (!normalized || unique.has(normalized.value))
      return;
...
function setThrowExpireOptions(items) {
  runtimeThrowExpireOptions = normalizeExpireOptions(items);
}
function getThrowExpireOptions() {
  return runtimeThrowExpireOptions.filter((item) => item.isActive).map((item) => ({ ...item }));
}
exports.getThrowExpireOptions = getThrowExpireOptions;
exports.setThrowExpireOptions = setThrowExpireOptions;
//# sourceMappingURL=../../.sourcemap/mp-weixin/common/throw-settings.js.map
